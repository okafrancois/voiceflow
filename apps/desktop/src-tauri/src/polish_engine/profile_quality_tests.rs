...
async fn french_profile_quality() {
    run_profile_quality(false).await;
}

#[tokio::test]
#[ignore = "requires an explicitly configured real inference server"]
async fn french_prose_regression() {
    run_profile_quality(true).await;
}

async fn run_profile_quality(prose_only: bool) {
    ...
    let cases = [
        Case {
            ...
            forbidden: &[], min_items: 3, min_blocks: 0, max_items: usize::MAX, shorter: false,
        },
        Case {
            name: "topic change",
            ...
            forbidden: &[], min_items: 0, min_blocks: 2, max_items: 0, shorter: false,
        },
        Case {
            ...
            forbidden: &["euh", "mardi", "pardon"], min_items: 0, min_blocks: 0, max_items: 0, shorter: false,
        },
        Case {
            ...
            forbidden: &["euh", "le truc", "cordialement", "bonjour"], min_items: 0, min_blocks: 0, max_items: 0, shorter: false,
        },
        Case {
            ...
            forbidden: &[], min_items: 0, min_blocks: 0, max_items: 0, shorter: true,
        },
        Case {
            ...
            forbidden: &[], min_items: 2, min_blocks: 0, max_items: usize::MAX, shorter: false,
        },
        Case {
            name: "dictated question stays a question",
            profiles: &["filler", "chat", "formal", "concise", "document", "agent"],
            ...
            forbidden: &[], min_items: 0, min_blocks: 0, max_items: 0, shorter: false,
        },
        Case {
            name: "ordinary narrative stays prose",
            profiles: &["filler", "chat", "formal", "concise", "document", "agent"],
            input: "J'ai essayé la nouvelle version ce matin et le résultat me semble meilleur. Le texte est plus lisible mais je voudrais encore vérifier le comportement avec un message un peu plus long.",
            required: &["version", "matin", "texte", "message"],
            forbidden: &[], min_items: 0, min_blocks: 0, max_items: 0, shorter: false,
        },
        Case {
            name: "context and request stay prose",
            profiles: &["filler", "chat", "formal", "concise", "document", "agent"],
            input: "Le bouton de sauvegarde ne répond plus depuis la mise à jour. Tu peux regarder ce qui se passe et me dire si ça vient du changement dans Settings.tsx ?",
            required: &["bouton", "sauvegarde", "Settings.tsx", "?"],
            forbidden: &[], min_items: 0, min_blocks: 0, max_items: 0, shorter: false,
        },
    ];
    ...
    for case in cases {
        if prose_only && (case.profiles.len() != 6 || case.max_items != 0 || case.min_blocks != 0) {
            continue;
        }
        if selected_case
            .as_deref()
        ...
        for profile in case.profiles {
            // The authorized rollback retains Structured Notes' historical list style.
            // The full quality suite still evaluates its stricter prose target.
            if prose_only && *profile == "document" {
                continue;
            }
            evaluated += 1;
            ...
            let (delivered, rejection) = accept_output(
                case.input,
                &output,
                TransformIntent::for_template(Some(profile)),
            );
            // The rollback gate checks what the user receives, including safety fallback.
            // The full suite continues to require the model itself to satisfy every rule.
            let output = if prose_only {
                println!("Delivered output (fallback={rejection:?}):\n{delivered}");
                delivered
            } else {
                output
            };
            ...
            let forbidden = case.forbidden.iter().any(|word| lower.contains(word))
                || (case.max_items == 0 && output.contains(". - "));
            let items = output
                .lines()
                .filter(|line| {
                    let line = line.trim();
            ...
            let paragraphs = output
                ...
                .count();
            if (!prose_only && rejection.is_some())
                || missing
                || forbidden
                || items < case.min_items
                || items > case.max_items
                || paragraphs < case.min_blocks
